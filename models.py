...
from tensorflow.compat.v1.keras.layers import Dense, BatchNormalization, Conv1D, MaxPooling1D, Flatten, Dropout, Lambda, Input, Conv2D, \
    Concatenate, Add, Subtract, Embedding, Reshape, GlobalAveragePooling1D, GlobalMaxPooling1D, TimeDistributed, \
    Masking, RepeatVector, Layer
...
class MixtureOutput(Model):
    def __init__(self, input_shape, n, d=1, activation='relu', eps=1e-4, bias_mu=1.8, bias_sigma=0.2,
        ...
        super().__init__(inputs=inp_masked, outputs=out, name=name)        #Model.__init__(self, inputs=inp_masked, outputs=out, name=name)
...
class NormalizedScaleEmbedding(Model):
    def __init__(self, input_shape, activation='relu', downsample=1, mlp_dims=(500, 300, 200, 150), eps=1e-8):
        #super().__init__()  # Panggil super() lebih awal    
        ...
        super().__init__(inputs=inp, outputs=out) #        Model.__init__(self, inputs=inp, outputs=out)
...
class Transformer(Model):
    def __init__(self, max_stations=32, emb_dim=500, layers=6, att_masking=False, hidden_dropout=0.0,
        ...
        # Definisikan input SEBELUM pemanggilan super()
        ...
        for _ in range(layers):
            attention_layer = MultiHeadSelfAttention(**mad_params)
            ffn_layer = PointwiseFeedForward(**ffn_params)  # Pastikan PointwiseFeedForward diganti jika perlu
            norm1_layer = LayerNormalization(**norm_params)
            norm2_layer = LayerNormalization(**norm_params)
            blocks.append((attention_layer, ffn_layer, norm1_layer, norm2_layer))

        for attention_layer, ffn_layer, norm1_layer, norm2_layer in blocks:
        ...
        # Pastikan input tetap sama
        inputs = inp
        if att_masking:
            inputs = [inp, att_mask]

        # Panggil `super().__init__()` SEBELUM ada atribut lain
        super().__init__(inputs=inputs, outputs=x)

        # Baru setelah super(), atribut tambahan bisa ditambahkan
        self.blocks = blocks
        self.built = True   
...
class PositionEmbedding(Layer):
    def __init__(self, wavelengths, emb_dim, borehole=False, rotation=None, rotation_anchor=None, **kwargs):
        super().__init__(**kwargs) #        super(PositionEmbedding, self).__init__(**kwargs)
...
class MultiHeadSelfAttention(Layer):
    def __init__(self, n_heads, infinity=1e6,
                 ...
                 att_dropout=0.0,
                 **kwargs):
        super().__init__(**kwargs) #        super(MultiHeadSelfAttention, self).__init__(**kwargs)        
    ...
    def call(self, x, mask=None):
        ...
        score = tf.matmul(q, k) / tf.sqrt(tf.cast(d_key, tf.float32)) #score = tf.matmul(q, k) / np.sqrt(d_key)  # (batch, n_heads, stations, stations)
        if mask is not None:
    ...
    def compute_mask(self, inputs, mask=None):
        if mask is None:#self.att_masking:
            return None#mask[0]
        return mask[0] if isinstance(mask, (list, tuple)) else mask#else:
            #return mask
...
class PointwiseFeedForward(Layer):
    def __init__(self, hidden_dim, kernel_initializer='glorot_uniform', bias_initializer='zeros', **kwargs):
        super().__init__(**kwargs)#          super(PointwiseFeedForward, self).__init__(**kwargs)
...
class LayerNormalization(Layer):
    def __init__(self, eps=1e-5, **kwargs):
        self.eps = eps
        super().__init__(**kwargs)#          super(LayerNormalization, self).__init__(**kwargs)
...
class AddEventToken(Layer):
    def __init__(self, fixed=True, init_range=None, **kwargs):
        ...
        super().__init__(**kwargs)#         super(AddEventToken, self).__init__(**kwargs)
...
class AddConstantToMixture(Layer):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)#          super(AddConstantToMixture, self).__init__(**kwargs)
...
class Masking_nd(Layer):
    def __init__(self, mask_value=0., axis=-1, nodim=False, **kwargs):
        super().__init__(**kwargs)#          super(Masking_nd, self).__init__(**kwargs)
...
class GetMask(Layer):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)#        super(GetMask, self).__init__(**kwargs)
...
class StripMask(Layer):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)#         super(StripMask, self).__init__(**kwargs)
...
class EnsembleEvaluateModel:
    ...
    def predict_generator(self, generator, **kwargs):
        ...
        preds = [model.predict(generator, **kwargs) for model in self.models]         
        ...
        for ens_id, model in enumerate(tmp_models):
            if self.loss_limit is not None:
                ...
                with open(hist_path, 'rb') as f:
                    hist = pickle.load(f)
                f.close()  # Tutup file setelah dibaca
