...
try:
    from tensorflow.compat.v1 import keras#import keras
except ImportError:
    keras = None
        ...
        def __init__(self, data, event_metadata, key='MA', batch_size=32, cutout=None,
            ...
            if magnitude_resampling > 1:
                ...
                for i in np.arange(min_upsample_magnitude, 9):
                    ...
                    #print(f"Processing index {ind}. Length of waveforms: {len(self.waveforms)}")
                    if ind.size == 0:  # Skip jika tidak ada indeks valid
                        continue
                    #if np.any(ind >= len(self.waveforms)):
                    #    raise ValueError(f"Indices out of range: {ind[ind >= len(self.waveforms)]}")
            ...
            if self.upsample_high_station_events is not None:
                ...
                for ind in self.base_indexes:
                    ...
                    #except IndexError:
                    #    raise ValueError(f"Index {ind} out of range for waveforms of length {len(self.waveforms)}")
                    n_stations = self.waveforms[ind].shape[0]
...
def generator_from_config(config, data, event_metadata, time, batch_size=64, sampling_rate=100, pga=False, dataset_id=None):
    ...
    noise_seconds = generator_params.get(0, {}).get('noise_seconds', 5)  
